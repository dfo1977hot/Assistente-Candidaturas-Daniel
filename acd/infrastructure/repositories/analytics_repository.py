...
class AnalyticsRepository:
    ...
    def get_latest_snapshot(self) -> AnalyticsSnapshot | None:
        with database_module.SessionLocal() as session:
            stmt = select(AnalyticsSnapshot).order_by(AnalyticsSnapshot.created_at.desc()).limit(1)
            return session.scalar(stmt)

    def create_report(
        self, title: str, period: str, content: str, *, format: str = "pdf"
    ) -> Report:
        with database_module.SessionLocal() as session:
    ...
    def create_recommendation(
        self, title: str, description: str, *, priority: str = "medium", action: str = ""
    ) -> AnalyticsRecommendation:
        with database_module.SessionLocal() as session:
            rec = AnalyticsRecommendation(
                title=title, description=description, priority=priority, action=action
            )
            session.add(rec)
            session.commit()
            session.refresh(rec)
        ...
        with database_module.SessionLocal() as session:
            if unread_only:
                stmt = (
                    ...
                    .where(AnalyticsRecommendation.read == False)
                    .order_by(AnalyticsRecommendation.created_at.desc())
                )
            else:
                stmt = select(AnalyticsRecommendation).order_by(
                    AnalyticsRecommendation.created_at.desc()
                )
            return list(session.scalars(stmt).all())

    def create_trend(
        self, metric_name: str, value: float, period: str, *, dimension: str = "global"
    ) -> Trend:
        with database_module.SessionLocal() as session:
    ...
    def get_trends(self, metric_name: str, dimension: str = "global") -> list[Trend]:
        with database_module.SessionLocal() as session:
            stmt = (
                select(Trend)
                .where((Trend.metric_name == metric_name) & (Trend.dimension == dimension))
                .order_by(Trend.created_at)
            )
            return list(session.scalars(stmt).all())
