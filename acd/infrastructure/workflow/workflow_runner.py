...
class WorkflowRunner:
    ...
    def __init__(self, repository: WorkflowRepository | None = None, event_bus: EventBus | None = None, dispatcher: CommandDispatcher | None = None) -> None:
    ...
    def run(self, execution: WorkflowExecution) -> dict[str, Any]:
        ...
        self.event_bus.publish("workflow_started", {"execution_id": execution.id, "workflow_id": execution.workflow_id})
        ...
        for step_index, step in enumerate(steps):
            ...
            self.event_bus.publish("step_started", {"execution_id": execution.id, "step_name": step_name})
            self.repository.create_log(execution.id, "info", f"Step {step_index}: {step_name} started")
            ...
            if command is None:
                ...
                self.event_bus.publish("step_failed", {"execution_id": execution.id, "step_name": step_name, "error": error_msg})
                break

            try:
                result = command.execute(context)
                self.event_bus.publish("step_completed", {"execution_id": execution.id, "step_name": step_name, "result": result})
                self.repository.create_log(execution.id, "info", f"Step {step_index}: {step_name} completed with {result}")
            except Exception as e:
                error_msg = str(e)
                execution.status = WorkflowState.FAILED.value
                self.repository.create_log(execution.id, "error", f"Step {step_index}: {step_name} failed: {error_msg}")
                self.event_bus.publish("step_failed", {"execution_id": execution.id, "step_name": step_name, "error": error_msg})
                break

        if execution.status == WorkflowState.EXECUTING.value:
            execution.status = WorkflowState.COMPLETED.value
            self.event_bus.publish("workflow_completed", {"execution_id": execution.id, "workflow_id": execution.workflow_id})
