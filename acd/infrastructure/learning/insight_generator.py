...
class InsightGenerator:
    ...
    def generate_from_pattern(
        ...
        """Generate an insight from pattern data."""
        ...
        return {
            ...
            "evidence_summary": {
                "evidence_count": pattern_data.get(
                    "evidence_count",
                    0,
                ),
                "criteria": pattern_data.get(
                    "criteria",
                    {},
                ),
                "impact_score": impact_score,
            },
            "data_points_analyzed": pattern_data.get(
                "evidence_count",
                0,
            ),
            "is_actionable": True,
    ...
    def _generate_title_and_description(
        ...
        description = (
            ...
            f"Based on {pattern_data.get('evidence_count', 0)} applications "
            f"with {pattern_data.get('confidence', 0.5) * 100:.0f}% confidence."
        )
    ...
    def _map_pattern_to_insight_type(
        self,
        pattern_type: str,
    ) -> str:
        ...
        mapping = {
            ...
            PatternType.RESUME_TYPE_SUCCESS.value: (
                InsightType.RESUME_OPTIMIZATION.value
            ),
            PatternType.CONVERSION_RATE.value: (
                InsightType.CONVERSION_TIP.value
            ),
            PatternType.RESPONSE_TIME.value: (
                InsightType.TIMING_INSIGHT.value
            ),
            PatternType.LETTER_EFFECTIVENESS.value: (
                InsightType.LETTER_IMPROVEMENT.value
            ),
        }
    ...
    def _describe_impact(
        ...
        return (
            f"Applying this insight could have a "
            f"{magnitude} impact "
            f"(estimated +{impact_pct:.0f}%) "
