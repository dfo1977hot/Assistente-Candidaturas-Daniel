...
class AnalyticsPage(BasePage):
    ...
    def create_trends_section(self) -> QWidget:
        ...
        trends_grid.setContentsMargins(
            0,
            0,
            0,
            0,
        ...
        ats_trend = trends.get(
            "ats",
            {},
        )

        ats_value = ats_trend.get(
            "trend",
            "estável",
        )

        ats_label = QLabel(
            f"ATS: {ats_value.upper()}"
        )
        ...
        ats_label.setStyleSheet(
            f"color: {ats_color}; "
            "font-weight: bold;"
        )

        trends_grid.addWidget(
            ats_label,
            0,
            0,
        )

        conversion_trend = trends.get(
            "conversion",
            {},
        )

        conversion_value = conversion_trend.get(
            "trend",
            "estável",
        )
        ...
        conversion_label = QLabel(
            "Conversão: "
            f"{conversion_value.upper()}"
        )

        conversion_label.setFont(
            QFont(
                "Arial",
                11,
            )
        ...
        conversion_color = (
            "#28a745"
            if conversion_value == "up"
            else "#dc3545"
            if conversion_value == "down"
            else "#666"
        )

        conversion_label.setStyleSheet(
            f"color: {conversion_color}; "
            "font-weight: bold;"
        )
    ...
    def create_recommendations_section(
        ...
        if recommendations:
            ...
            no_recommendation_label = QLabel(
                "Nenhuma recomendação no momento."
            )

            no_recommendation_label.setStyleSheet(
                "color: #999;"
            )
