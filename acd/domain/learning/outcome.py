...
class Outcome(Base):
    ...
    application_id: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True, comment="Related application ID")
    ...
    context: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict, nullable=False, comment="Additional context")
    time_to_outcome: Mapped[int | None] = mapped_column(Integer, nullable=True, comment="Days until outcome (if applicable)")
    quality_feedback: Mapped[str] = mapped_column(Text, default="", nullable=False, comment="User feedback on quality")
    relevance_score: Mapped[float] = mapped_column(Numeric(precision=5, scale=4), nullable=False, default=0.5)
    engagement_level: Mapped[str] = mapped_column(String(20), default="medium", nullable=False, comment="High/Medium/Low")
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False, index=True)
    recorded_date: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False, comment="When outcome occurred")
