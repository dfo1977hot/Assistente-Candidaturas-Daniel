...
class CurriculumService:
    ...
    def __init__(
        ...
        self.document_service = CurriculumDocumentService(
            self.repository
        )
    ...
    def delete_curriculum(
        ...
        *,
        ...
        curriculum = self.repository.get_by_id(curriculum_id)
        document_path = (
            self.document_service.resolve_path(
                curriculum.file_relative_path
            )
    ...
    def duplicate_curriculum(
        ...
        """Cria nova versão manual e duplica o arquivo físico."""
        ...
        duplicated = Curriculum(
            ...
            structured_content_json=base.structured_content_json,
        ...
        self.repository.create_version(
            ...
            CurriculumVersion(
                ...
                file_path="",
                file_type=(
                    base.file_extension or ""
                ).removeprefix("."),
            ),
        )

        if base.file_relative_path:
            self.document_service.duplicate_document(
                base,
                created,
            )
            created = (
    ...
    def create_optimized_curriculum(
        ...
        *,
        ...
        """Materializa otimização sem alterar o currículo original."""
        ...
        next_version = self._next_optimized_version(
            source.version
        )
        ...
        existing = next(
            (
                curriculum
                for curriculum in self.repository.search(
                    source.name
                )
                if (
                    curriculum.name == source.name
                    and curriculum.version == next_version
                )
            ),
            None,
        )
        ...
        optimized = Curriculum(
            ...
            structured_content_json=(
                optimized_structured_content
                or source.structured_content_json
            ),
            version=next_version,
        ...
        versioned_file_name = (
            self.document_service.versioned_original_name(
                source.file_original_name or source.name,
                next_version,
            )
        )
        ...
        try:
            if source.file_relative_path:
                if optimized_content:
                    self.document_service.materialize_optimized_document(
                        source,
                        created,
                    ...
                    self.document_service.duplicate_document(
                        source,
                        created,
                        original_name=versioned_file_name,
                    )
        ...
        logger.info(
            "Currículo otimizado cadastrado: "
            "origem=%s novo=%s versão=%s arquivo=%s",
            source_curriculum_id,
            created.id,
            next_version,
            created.file_original_name,
        )
    ...
    def attach_document(
        ...
        return self.document_service.attach_document(
            curriculum_id,
            source_path,
        )
    ...
    def resolve_document_path(
        ...
        if (
            curriculum is None
            or not curriculum.file_relative_path
        ):
            return None

        path = self.document_service.resolve_path(
            curriculum.file_relative_path
        )
    ...
    def suggested_export_filename(
        ...
        """Retorna o nome sugerido para exportação do currículo."""
        curriculum = self.repository.get_by_id(curriculum_id)

        if curriculum is None:
            return "Curriculo.docx"

        if curriculum.file_original_name:
            original = Path(curriculum.file_original_name)

            if original.suffix.lower() == ".docx":
                return original.name

        base_name = curriculum.name.strip() or "Curriculo"

        if curriculum.version and curriculum.version != "v1.0":
            return self.document_service.versioned_original_name(
                f"{base_name}.docx",
                curriculum.version,
            )
    ...
    def open_document(
        ...
        self.document_service.open_document(
            curriculum_id
        )
    ...
    def activate_curriculum(
        ...
        curriculum = self.repository.get_by_id(
            curriculum_id
        )
        ...
        if (
            ...
            and current_default.id != curriculum_id
        ):
    ...
    def associate_to_application(
        ...
        *,
        ...
        application.curriculum_version = (
            curriculum.version
        )
        ...
        logger.info(
            "Currículo associado à candidatura: %s",
            application_id,
        )
    ...
    def get_statistics(self) -> dict[str, int]:
        ...
        from acd.infrastructure.repositories.application_repository import (
    ...
    def _next_version(self, version: str) -> str:
        """Mantém versionamento menor para duplicações manuais."""
    ...
    @staticmethod
    def _next_optimized_version(
        version: str,
    ) -> str:
        """Currículos otimizados avançam versão principal: v1.0 -> v2.0."""
        normalized = version.strip()
    ...
    def _get_application(
        ...
        return ApplicationRepository().get_by_id(
            application_id
        )
