    ...
    def _build_context(
        ...
        *,
        ...
        ats_result = self.ats_service.compare_curriculum(
            curriculum=persisted_curriculum,
...
class ResumeGenerationService(BaseGenerationService):
    ...
    def __init__(
        ...
        prompt_builder: PromptBuilder | None = None,
        ats_service: ATSService | None = None,
        document_service: CurriculumDocumentService | None = None,
    ) -> None:
        super().__init__(
        ...
        self.document_service = (
            document_service
            or CurriculumDocumentService(CurriculumRepository())
        )
    ...
    def generate_resume(
        ...
        *,
        ...
        return self.generate_resume_from_context(
    ...
    def generate_resume_from_context(
        ...
        *,
        ...
        context = self._with_document_blocks(
            curriculum_id,
            context,
        )
        ...
        version = self.repository.save_resume_version(
            ResumeVersion(
                ...
                content=response,
                explanation=(
                    "Destacadas competências alinhadas à vaga "
    ...
    def _with_document_blocks(
        self,
        curriculum_id: int,
        context: PromptContext,
    ) -> PromptContext:
        if context.document_blocks:
            return context

        try:
            blocks = self.document_service.extract_text_blocks(
                curriculum_id
            )
        except (CurriculumDocumentError, OSError):
            return context
...
class CoverLetterGenerationService(BaseGenerationService):
    ...
    def _next_version(self, curriculum_id: int) -> str:
        versions = self.repository.get_cover_letter_versions(
            curriculum_id
        )
        if not versions:
            return "v1.0"
