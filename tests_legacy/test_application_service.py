...
def service_setup(db_session):
    """
    Reutiliza a infraestrutura compartilhada de testes (db_session),
    evitando recriar o banco para este módulo.
    """
    company_service = CompanyService(repository=CompanyRepository())
    company_id = company_service.create_company(
        name="Acme",
        city="São Paulo",
    ).id
    ...
    yield (
        ApplicationService(repository=ApplicationRepository()),
        job_id,
        company_id,
    )
...
def test_application_service_creates_and_changes_status(service_setup):
    ...
    updated = application_service.change_status(
        created.id,
