...
def test_pyproject_is_the_official_metadata_source() -> None:
    ...
    assert {item.split(">=", 1)[0] for item in project["dependencies"]} == {
        "keyring", "openai", "openpyxl", "pandas", "playwright", "psutil", "pydantic",
        "pywin32", "PySide6", "python-docx", "SQLAlchemy"
    }
...
def test_theme_manager_loads_packaged_stylesheet_independently_of_cwd(monkeypatch, tmp_path) -> None:
...
def test_gitignore_protects_local_state_without_hiding_official_examples() -> None:
    ...
    assert {
        ".venv/", ".pytest_cache/", ".coverage", ".coverage.*", "build/", "dist/",
        "*.egg-info/", "data/database/*.db", "*.db-wal", "*.db-shm", "*.db-journal",
        "*.sqlite", "*.sqlite3", "*.journal", ".env", ".env.*", "!.env.example",
        ".local/artifacts/", ".sprint-*.stdout.log", ".sprint-*.stderr.log",
        ".sprint-*.wrapper.ps1", ".sprint-*.gate.pid", ".fast-quality-gate-*",
        ".coverage-*.json",
    } <= rules
