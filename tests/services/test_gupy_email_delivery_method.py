...
class _Page:
    ...
    def get_by_role(self, _role: str, name: str, **_kwargs: object) -> _Locator:
    ...
    def get_by_label(self, _value: str, **_kwargs: object) -> _Locator:
        return self.email

    def get_by_placeholder(self, _value: str, **_kwargs: object) -> _Locator:
        return self.email
    ...
    def locator(self, selector: str) -> _Locator:
        if selector in {
            ...
            return self.buttons.setdefault("Entrar sem senha", _Locator("Entrar sem senha"))
        return self.email
...
def test_gupy_clicks_receive_link_by_email_before_polling_imap() -> None:
    ...
    browser = _Browser(magic_link_service=magic_link_service)  # type: ignore[arg-type]
    page = _Page()
    progress_events: list[object] = []

    browser._prepare_gupy(
        page,
        ApplicantProfile("Daniel", "dfo1977@terra.com.br"),
    ...
    assert page.visited == ["https://login.gupy.io/magic?token=test"]
